...
- **Declared subsystems run locally, and only those.** `apply` records what your `sshd_config` `Subsystem` lines resolve to (asking sshd itself, via `sshd -T`), and the shim runs exactly those commands directly instead of relaying them — in practice that means `sftp`. Anything else is relayed, including an `scp` invoked as an ordinary command. That is a throughput choice, not a correctness one: the relay carries arbitrary bytes unchanged in both directions, so a relayed transfer still works, it just costs latency. If you change a `Subsystem` line, re-run `apply` or the shim will keep matching the old one.
